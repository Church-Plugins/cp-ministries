...
class Settings {
	...
	/**
	 * Only make one instance of \CP_Ministries\Settings
	 *
	 * @return Settings
	 */
	...
	public function register_main_options_metabox() {
		...
		$post_type = CP_Ministries()->setup->post_types->item_type_enabled() ? CP_Ministries()->setup->post_types->item_type->post_type : CP_Ministries()->setup->post_types->item->post_type;
		/**
		...
		$main_options->add_field( array(
			'name'    => __( 'Primary Color', 'cp-ministries' ),
			'desc'    => __( 'The primary color to use in the templates.', 'cp-ministries' ),
			'id'      => 'color_primary',
			'type'    => 'colorpicker',
			'default' => '#333333',
		) );
		...
		$main_options->add_field( array(
			'name'         => __( 'Site Logo', 'cp-ministries' ),
			'desc'         => sprintf( __( 'The logo to use for %s.', 'cp-ministries' ), CP_Ministries()->setup->post_types->item->plural_label ),
			'id'           => 'logo',
			'type'         => 'file',
		...
		$main_options->add_field( array(
			'name'         => __( 'Default Thumbnail', 'cp-ministries' ),
			'desc'         => sprintf( __( 'The default thumbnail image to use for %s.', 'cp-ministries' ), CP_Ministries()->setup->post_types->item->plural_label ),
		...
		if ( CP_Ministries()->setup->post_types->item_type_enabled() ) {
			$this->item_type_options();
		}

		if ( CP_Ministries()->setup->post_types->speaker_enabled() ) {
	...
	protected function item_options() {
		...
		$args = array(
			...
			'parent_slug'  => 'cpl_main_options',
			'tab_group'    => 'cpl_main_options',
			'tab_title'    => CP_Ministries()->setup->post_types->item->plural_label,
			'display_cb'   => [ $this, 'options_display_with_tabs' ],
		);
		...
		$options->add_field( array(
			'name'    => __( 'Singular Label', 'cp-ministries' ),
			'id'      => 'singular_label',
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->item->single_label,
		) );

		$options->add_field( array(
			'name'    => __( 'Plural Label', 'cp-ministries' ),
			'id'      => 'plural_label',
			'desc'    => __( 'Caution: changing this value will also adjust the url structure and may affect your SEO.', 'cp-ministries' ),
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->item->plural_label,
		) );
	...
	protected function item_type_options() {
		...
		$args = array(
			...
			'parent_slug'  => 'cpl_main_options',
			'tab_group'    => 'cpl_main_options',
			'tab_title'    => CP_Ministries()->setup->post_types->item_type->plural_label,
			'display_cb'   => [ $this, 'options_display_with_tabs' ],
		);
		...
		$options->add_field( array(
			...
			'id'      => 'singular_label',
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->item_type->single_label,
		) );
		...
		$options->add_field( array(
			...
			'desc'    => __( 'Caution: changing this value will also adjust the url structure and may affect your SEO.', 'cp-ministries' ),
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->item_type->plural_label,
		) );
	...
	protected function speaker_options() {
		...
		$args = array(
			...
			'parent_slug'  => 'cpl_main_options',
			'tab_group'    => 'cpl_main_options',
			'tab_title'    => CP_Ministries()->setup->post_types->speaker->plural_label,
			'display_cb'   => [ $this, 'options_display_with_tabs' ],
		);
		...
		$options->add_field( array(
			...
			'id'      => 'singular_label',
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->speaker->single_label,
		) );
		...
		$options->add_field( array(
			...
			'id'      => 'plural_label',
			'type'    => 'text',
			'default' => CP_Ministries()->setup->post_types->speaker->plural_label,
		) );
	...
	protected function advanced_options() {
		...
		$advanced_options->add_field( array(
			'name'    => __( 'Enable' ) . ' ' . CP_Ministries()->setup->post_types->item_type->plural_label,
			'id'      => 'item_type_enabled',
			'type'    => 'radio_inline',
			'default' => 1,
			'options' => [
				1 => __( 'Enable', 'cp-ministries' ),
				0 => __( 'Disable', 'cp-ministries' ),
			]
		) );

		$advanced_options->add_field( array(
			'name'    => __( 'Enable' ) . ' ' . CP_Ministries()->setup->post_types->speaker->plural_label,
			'id'      => 'speaker_enabled',
			'type'    => 'radio_inline',
			'default' => 1,
			'options' => [
